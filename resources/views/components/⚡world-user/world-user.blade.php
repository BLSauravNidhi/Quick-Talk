...
<div>
    <div class=" w-full max-w-80 h-fit grid grid-cols-[70px_auto] items-center gap-3 my-shadow rounded-md py-3 px-3 overflow-hidden">
        <img src="{{ $world_user->profile && file_exists(public_path('storage/' . $world_user->profile))
                ? asset('storage/' . $world_user->profile) 
                : ($world_user->profile ? $world_user->profile : asset('images/anonymous_user.webp')) }}" alt="Profile" class=" rounded-full w-15 h-15 object-cover mx-auto" referrerpolicy="no-referrer">
