...
<div class="w-full">
    <a href="" class=" w-full h-fit flex items-center gap-3 py-3 px-3 overflow-hidden bg-gray-300">
        <img src="{{ $request->senderInfo->profile && file_exists(public_path('storage/' . $request->senderInfo->profile))
                ? asset('storage/' . $request->senderInfo->profile) 
                : ($request->senderInfo->profile ? $request->senderInfo->profile : asset('images/anonymous_user.webp')) }}" alt="Profile" class=" rounded-full w-15 h-15 object-cover" referrerpolicy="no-referrer">
        <div class="w-full">
